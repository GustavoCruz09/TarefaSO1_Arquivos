...
public class ArquivosController implements IArquivosController {
	...
	public void readDir(String path) throws IOException {
		File dir = new File(path); // path é o caminho do diretório
		if (dir.exists() && dir.isDirectory()) { // verifica se existe e é um diretório
			File[] files = dir.listFiles(); // cria um vetor com todos os arquivos e diretórios
			for (File f : files) {
				if (f.isFile()) {
	...
	public void createFile(String path, String nome) throws IOException {
		...
		if (dir.exists() && dir.isDirectory()) {
			...
			FileWriter fileWriter = new FileWriter(arq, existe); // responsavel por abrir arquivo e definir se será
																	// appendu write
